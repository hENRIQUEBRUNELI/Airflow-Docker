-- ============================================================
-- 10 KPIs DO DATA WAREHOUSE DE VENDAS — AdventureWorks
-- Star Schema · PostgreSQL
-- ============================================================

-- KPI 1: Receita Total de Vendas por Período (com variação vs período anterior)
SELECT t.ano, t.mes, t.nome_mes,
       SUM(f.receita_liquida) AS receita_total,
       SUM(f.receita_liquida) - LAG(SUM(f.receita_liquida)) OVER (ORDER BY t.ano, t.mes) AS variacao_periodo_anterior,
       ROUND((SUM(f.receita_liquida) - LAG(SUM(f.receita_liquida)) OVER (ORDER BY t.ano, t.mes))
             / NULLIF(LAG(SUM(f.receita_liquida)) OVER (ORDER BY t.ano, t.mes), 0) * 100, 2) AS variacao_pct
FROM fato_vendas f JOIN dim_tempo t ON f.sk_tempo = t.sk_tempo
...
SELECT t.ano, t.mes, t.nome_mes,
       ...
       SUM(f.receita_liquida)      AS receita_total,
       ROUND(SUM(f.receita_liquida) / NULLIF(COUNT(DISTINCT f.nk_pedido), 0), 2) AS ticket_medio
FROM fato_vendas f JOIN dim_tempo t ON f.sk_tempo = t.sk_tempo
GROUP BY t.ano, t.mes, t.nome_mes
ORDER BY t.ano, t.mes;
...
FROM fato_vendas f JOIN dim_produto p ON f.sk_produto = p.sk_produto
WHERE p.registro_atual = TRUE
GROUP BY p.categoria, p.subcategoria
ORDER BY margem_pct DESC;

-- KPI 4: Ranking de Produtos Mais Vendidos (Top 10 por Receita)
SELECT RANK() OVER (ORDER BY SUM(f.receita_liquida) DESC) AS ranking,
...
SELECT v.nome_completo AS vendedor, v.territorio, t.ano,
       SUM(f.receita_liquida) AS receita_realizada,
       MAX(v.cota_anual)      AS cota_anual,
       ROUND(SUM(f.receita_liquida) / NULLIF(MAX(v.cota_anual), 0) * 100, 2) AS pct_atingimento_cota,
       CASE WHEN SUM(f.receita_liquida) >= MAX(v.cota_anual)       THEN 'Atingiu'
            WHEN SUM(f.receita_liquida) >= MAX(v.cota_anual) * 0.8 THEN 'Próximo'
            ELSE 'Abaixo' END AS status_cota
FROM fato_vendas f
JOIN dim_vendedor v ON f.sk_vendedor = v.sk_vendedor
JOIN dim_tempo    t ON f.sk_tempo    = t.sk_tempo
WHERE v.registro_atual = TRUE
GROUP BY v.nome_completo, v.territorio, t.ano
ORDER BY t.ano, pct_atingimento_cota DESC;

-- KPI 6: Receita por Território / Região (com % do ano)
SELECT ter.grupo AS regiao_grupo, ter.pais, ter.nome_territorio, t.ano,
       COUNT(DISTINCT f.nk_pedido) AS total_pedidos,
       SUM(f.quantidade)           AS qtd_itens,
       SUM(f.receita_liquida)      AS receita_total,
       ROUND(SUM(f.receita_liquida) / SUM(SUM(f.receita_liquida)) OVER (PARTITION BY t.ano) * 100, 2) AS pct_receita_ano
FROM fato_vendas f
...
SELECT pr.descricao AS promocao, pr.tipo_desconto, pr.categoria,
...
FROM fato_vendas f JOIN dim_promocao pr ON f.sk_promocao = pr.sk_promocao
GROUP BY pr.descricao, pr.tipo_desconto, pr.categoria
ORDER BY total_desconto_concedido DESC;

-- KPI 8: Crescimento de Receita Ano a Ano (YoY)
WITH receita_anual AS (
    SELECT t.ano, SUM(f.receita_liquida) AS receita_total
    FROM fato_vendas f JOIN dim_tempo t ON f.sk_tempo = t.sk_tempo
    GROUP BY t.ano
)
SELECT ano, receita_total,
       ...
       ROUND((receita_total - LAG(receita_total) OVER (ORDER BY ano))
             / NULLIF(LAG(receita_total) OVER (ORDER BY ano), 0) * 100, 2) AS crescimento_yoy_pct
FROM receita_anual ORDER BY ano;
...
WITH rfm AS (
    SELECT f.sk_cliente,
           ...
           (current_date - MAX(t.data_completa)) AS dias_desde_ultima_compra
    FROM fato_vendas f JOIN dim_tempo t ON f.sk_tempo = t.sk_tempo
    GROUP BY f.sk_cliente
)
SELECT c.nome_completo, c.tipo_cliente, c.pais,
       rfm.ultima_compra,
       ...
       CASE WHEN rfm.dias_desde_ultima_compra <= 365 AND rfm.frequencia >= 5 AND rfm.valor_total >= 10000 THEN 'Premium'
            WHEN rfm.dias_desde_ultima_compra <= 730 AND rfm.frequencia >= 2 THEN 'Recorrente'
            ELSE 'Ocasional' END AS segmento_cliente
FROM rfm JOIN dim_cliente c ON rfm.sk_cliente = c.sk_cliente
WHERE c.registro_atual = TRUE
ORDER BY rfm.valor_total DESC;

-- KPI 10: Sazonalidade de Vendas por Trimestre
SELECT t.ano, t.trimestre,
       CONCAT('Q', t.trimestre, '/', t.ano) AS periodo,
       COUNT(DISTINCT f.nk_pedido) AS total_pedidos,
       SUM(f.quantidade)           AS qtd_itens,
       SUM(f.receita_liquida)      AS receita_total,
       ROUND(AVG(f.receita_liquida), 2) AS receita_media_por_item,
       ROUND(SUM(f.receita_liquida) / SUM(SUM(f.receita_liquida)) OVER (PARTITION BY t.ano) * 100, 2) AS pct_receita_no_ano,
       ROUND(SUM(f.receita_liquida) / NULLIF(AVG(SUM(f.receita_liquida)) OVER (PARTITION BY t.trimestre), 0) * 100, 2) AS indice_sazonalidade
FROM fato_vendas f JOIN dim_tempo t ON f.sk_tempo = t.sk_tempo
